...
function initMap () {
	...
	function onLocationFound(e) {
		...
		currentLocationMarker = new L.marker(e.latlng, {draggable: true, icon: markerIcon}).addTo(map).bindPopup('Move me to search').openPopup();
...
function runSearch () {
	require([
		...
		"dojo/_base/xhr",
		"dojo/domReady!"], 
	function(JSON, dom, domConstruct, on, RoundRectList, ListItem, xhr){
		console.log("****Running runSearch()****");
		
		console.log("**********Map spin true***********");
		map.spin(true);
	...
	console.log("**********Map spin false***********");
	map.spin(false);
	currentLocationMarker.setOpacity(1);
}

dojo.addOnLoad( function() {
  // attach on click to id="convert"
  dojo.query('#convert').onclick( function(evt) { 
	refreshMap();
	});
});

dojo.addOnLoad( function() {
  // attach on click to id="tab-search"
